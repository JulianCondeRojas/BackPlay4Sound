...
router.put("/edit/:id", [validateObjectId, auth], async (req, res) => {
	...
	if (!user._id.equals(playlist.user))
		return res.status(403).send({ message: "¡El usuario no tiene permisos para editar!" });
...
router.put("/add-song", auth, async (req, res) => {
	...
	if (!user._id.equals(playlist.user))
		return res.status(403).send({ message: "¡El usuario no tiene permisos para añadir!" });
...
router.put("/remove-song", auth, async (req, res) => {
	...
	if (!user._id.equals(playlist.user))
		return res
			.status(403)
			.send({ message: "¡El usuario no tiene permisos para eliminar!" });

	const index = playlist.songs.indexOf(req.body.songId);
	playlist.songs.splice(index, 1);
	await playlist.save();
	res.status(200).send({ data: playlist, message: "Eliminada de la playlist" });
});
...
router.get("/:id", [validateObjectId, auth], async (req, res) => {
	const playlist = await PlayList.findById(req.params.id);
	if (!playlist) return res.status(404).send("No encontrada");
...
router.delete("/:id", [validateObjectId, auth], async (req, res) => {
	...
	res.status(200).send({ message: "Eliminada de la biblioteca" });
});
